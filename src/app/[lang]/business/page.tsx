...
export default async function BusinessPage({
  ...
  const base = `/${lang}`;
  const page = await getPage("business");
  const subpages = await getPagesBySection("business");
  ...
  const subRoutes: Record<string, string> = {
    ...
    products: `${base}/business/products`,
    electronics: `${base}/business/electronics`,
    lab: `${base}/business/lab`,
    global: `${base}/business/global`,
  };
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
            {subpages
              ...
              .map((s) => (
                <Link
                  key={s.id}
                  href={subRoutes[s.id] || `${base}/business/${s.id}`}
