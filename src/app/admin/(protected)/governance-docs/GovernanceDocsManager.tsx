...
import { GOVERNANCE_DOCS_FIELDS, GOVERNANCE_DOCS_COLUMNS } from "@/lib/admin-fields";

interface Props {
  initialData: Record<string, unknown>[];
}

export function GovernanceDocsManager({ initialData }: Props) {
  ...
  const handleSave = useCallback(
    async (formData: Record<string, unknown>) => {
      if (editingRow) {
        const res = await fetch(`/api/admin/governance_docs/${editingRow.id}`, {
          method: "PATCH",
        ...
        const res = await fetch("/api/admin/governance_docs", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify(formData),
        });
  ...
  const handleDelete = useCallback(
    async (id: string | number) => {
      const res = await fetch(`/api/admin/governance_docs/${id}`, { method: "DELETE" });
      if (res.status === 401) { router.push("/admin/login"); return; }
    ...
    <>
      ...
      {modalOpen && (
        <AdminModal
          ...
          onSave={handleSave}
          title={editingRow ? "編輯企業管治文件" : "新增企業管治文件"}
