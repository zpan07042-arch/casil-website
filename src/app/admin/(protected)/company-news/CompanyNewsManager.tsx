...
import { COMPANY_NEWS_FIELDS, COMPANY_NEWS_COLUMNS } from "@/lib/admin-fields";

interface Props {
  initialData: Record<string, unknown>[];
}

export function CompanyNewsManager({ initialData }: Props) {
  ...
  const handleSave = useCallback(
    async (formData: Record<string, unknown>) => {
      if (editingRow) {
        const res = await fetch(`/api/admin/company_news/${editingRow.id}`, {
          method: "PATCH",
        ...
        const res = await fetch("/api/admin/company_news", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify(formData),
        });
  ...
  const handleDelete = useCallback(
    async (id: string | number) => {
      const res = await fetch(`/api/admin/company_news/${id}`, { method: "DELETE" });
      if (res.status === 401) { router.push("/admin/login"); return; }
    ...
    <>
      ...
      {modalOpen && (
        <AdminModal
          ...
          onSave={handleSave}
          title={editingRow ? "編輯公司資訊" : "新增公司資訊"}
